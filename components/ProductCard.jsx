      ...
      <div className="p-photo" style={{ '--tint': `var(--tint-${tint})`, '--tint-deep': `var(--tint-${tint}-deep)` }}>
        ...
        {(onEdit || onDelete) && (
          <div className="p-admin">
            ...
            {onDelete && (
              <button type="button" className="icon-btn danger" aria-label="Eliminar" onClick={() => setConfirming(true)}>
        ...
        {confirming && (
          <div className="confirm-del-sm">
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="button" className="btn-amber" style={{ border: 'none' }} onClick={() => { onDelete(item); setConfirming(false); }}>Sí, eliminar</button>
              <button type="button" className="btn-ghost" style={{ border: '1px solid rgba(255,255,255,0.4)', background: 'transparent', color: '#fff' }} onClick={() => setConfirming(false)}>Cancelar</button>
            </div>
          </div>
        )}
      </div>
